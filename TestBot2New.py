...
time_start =  1584104400

time_frozen = 1584190800

time_finish = 1584190800
...
def bot_messages(request, mycursor, mydb):
    ...
    try:
        ...
        sql = ("SELECT telegram_id FROM bot_users WHERE is_admin = 0")
        mycursor.execute(sql)
        myresult = mycursor.fetchall()
        message = ("Произошло трагическое крушение бота. Сохраняйте спокойствие, администратор уже решает эту проблему.")
        if myresult:
            for i in myresult:
                send_mess_nokeyboard(i[0], message)
        sql = ("SELECT telegram_id FROM bot_users WHERE is_admin = 1")
        mycursor.execute(sql)
        myresult = mycursor.fetchall()
        message = ('Бот упал! Сделай с этим что-то немедленно!')
        for i in myresult:
            send_mess_nokeyboard(i[0], message)
        return 0
...
def main():
    ...
    flag = 1
    while flag:
        flag = bot_messages(url, mycursor, mydb)
