...
time_start =  1583082600

time_frozen = 1583083500

time_finish = 1583083800
...
questions = ['Сколько будет 2+2?',
             'В каком году началась Вторая Мировая война?',
             'Какая фамилия была у главного героя романа "Идиот"?',
             'Как называется физическое явления, смысл которого заключается в выравнивании концентрации молекул в замкнутом объеме?',
             'Какой ник у этого бота?',
             'Сколько будет 2+2?',
             'В каком году началась Вторая Мировая война?',
             'Какая фамилия была у главного героя романа "Идиот"?',
             'Как называется физическое явления, смысл которого заключается в выравнивании концентрации молекул в замкнутом объеме?',
                ...
                if not is_admin:
                    if not is_reg:
                        if not status or status == 1:
                            ...
                            if first_result['message']['text']=='Нет':
                                ...
                                message = ('Для окончания регистрации введите, пожалуйста, информацию о себе в формате: Имя Фамилия. \n' +
                                           'Например: Михаил Алексеев.' )
                                send_mess(person_id, message, keyboard_cancel)
                            ...
                            if capitan==1:
                                if status==1:
                                    ...
                                    if first_result['message']['text']!='В главное меню':
                                        ...
                                        send_mess(person_id,'Действие отменено.', keyboard_capitan)
                                        update_status = ("UPDATE bot_users SET status = 1 WHERE telegram_id = %(tg_id)s")
                                        mycursor.execute(update_status, {'tg_id': person_id})
                                        mydb.commit()
                                elif status==3:
                    ...
                    if status == 1:
                        ...
                        if first_result['message']['text']!='Назад':
                            ...
                            mycursor.execute(update_status, {'tg_id': person_id})
                            mydb.commit()
                            sql = ('SELECT `telegram_id` FROM `bot_users` WHERE `in_game` = 1')
                            mycursor.execute(sql)
                            myresult = mycursor.fetchall()
                            if not myresult:
